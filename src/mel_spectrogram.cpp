...
bool load_wav(const std::string& path, std::vector<float>& samples, int& sample_rate) {
    ...
    while (file.good()) {
        ...
        if (strncmp(chunk_id, "fmt ", 4) == 0) {
            ...
            auto downmix_to_mono = [&](auto sample_at) {
                for (int i = 0; i < num_samples; ++i) {
                    float sum = 0.0f;
                    for (int c = 0; c < num_channels; ++c) {
                        sum += sample_at(i * num_channels + c);
                    }
            ...
            if (audio_format == 1) {
                if (bits_per_sample != 16) {
                    fprintf(stderr, "Error: Only 16-bit integer PCM supported for WAV format 1 (got %d bits)\n", bits_per_sample);
                    return false;
                }

                std::vector<int16_t> raw_samples(num_samples * num_channels);
                file.read(reinterpret_cast<char*>(raw_samples.data()), chunk_size);
                downmix_to_mono([&](int idx) {
                    return raw_samples[static_cast<size_t>(idx)] / 32768.0f;
                });
                ...
                if (bits_per_sample == 32) {
                    ...
                    file.read(reinterpret_cast<char*>(raw_samples.data()), chunk_size);
                    downmix_to_mono([&](int idx) {
                        return raw_samples[static_cast<size_t>(idx)];
                    });
                } else if (bits_per_sample == 64) {
                    std::vector<double> raw_samples(num_samples * num_channels);
                    file.read(reinterpret_cast<char*>(raw_samples.data()), chunk_size);
                    downmix_to_mono([&](int idx) {
                        return static_cast<float>(raw_samples[static_cast<size_t>(idx)]);
                    });
                } else {
                    fprintf(stderr, "Error: Only 32-bit or 64-bit float WAV supported for format 3 (got %d bits)\n", bits_per_sample);
                    return false;
                }
            } else {
                fprintf(stderr, "Error: Unsupported WAV format %d (supported: PCM=1, IEEE float=3)\n", audio_format);
                return false;
            }
...
bool log_mel_spectrogram(const float* samples, int n_samples,
    ...
    const int n_samples_orig = n_samples;

    // Match the mtmd Qwen3-ASR preprocessor:
    // - short inputs are zero-padded to avoid stage-1/stage-2 padding failures
    // - 30 seconds of zero tail padding are added
    // - frame_size / 2 samples are reflect-padded at the front
    std::vector<float> short_input_pad;
    const int min_samples = frame_size + frame_step;
    if (n_samples < min_samples) {
        short_input_pad.resize(min_samples, 0.0f);
        std::copy(samples, samples + n_samples, short_input_pad.begin());
        samples = short_input_pad.data();
        n_samples = min_samples;
    }

    const int stage_1_pad = QWEN_SAMPLE_RATE * QWEN_CHUNK_SIZE;
    const int stage_2_pad = frame_size / 2;

    std::vector<float> samples_padded(static_cast<size_t>(n_samples + stage_1_pad + stage_2_pad * 2), 0.0f);
    std::copy(samples, samples + n_samples, samples_padded.begin() + stage_2_pad);
    std::reverse_copy(samples + 1, samples + 1 + stage_2_pad, samples_padded.begin());

    const int total_frames = (static_cast<int>(samples_padded.size()) - frame_size) / frame_step + 1;
    const int n_real_frames = std::min(n_samples_orig / frame_step + 1, total_frames);

    MelSpectrogram full_mel;
    full_mel.n_mel = filters.n_mel;
    full_mel.n_len = total_frames;
    full_mel.n_len_org = n_samples_orig;
    full_mel.data.resize(static_cast<size_t>(full_mel.n_mel) * full_mel.n_len);
    const float* hann = global_cache.hann_window_f;
    n_threads = std::max(1, n_threads);

    if (n_threads > 1) {
        std::vector<std::thread> workers(static_cast<size_t>(n_threads - 1));
        for (int iw = 0; iw < n_threads - 1; ++iw) {
            workers[static_cast<size_t>(iw)] = std::thread(
                log_mel_spectrogram_worker,
                iw + 1,
                hann,
                std::cref(samples_padded),
                static_cast<int>(samples_padded.size()),
                frame_size,
                frame_step,
                n_threads,
                std::cref(filters),
    ...
    for (float & value : full_mel.data) {
        if (value < mmax) {
            value = static_cast<float>(mmax);
        }
        value = (value + 4.0f) / 4.0f;
    }

    mel.n_mel = full_mel.n_mel;
    mel.n_len = n_real_frames;
    mel.n_len_org = n_real_frames;
    mel.data.resize(static_cast<size_t>(mel.n_mel) * mel.n_len);

    for (int j = 0; j < mel.n_mel; ++j) {
        const float * src = full_mel.data.data() + static_cast<size_t>(j) * full_mel.n_len;
        float * dst = mel.data.data() + static_cast<size_t>(j) * mel.n_len;
        std::copy(src, src + mel.n_len, dst);
    }
